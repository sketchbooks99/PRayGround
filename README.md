...
## テクスチャ
---
`ref`: [テクスチャメモリ](http://www.slis.tsukuba.ac.jp/~fujisawa.makoto.fu/cgi-bin/wiki/index.php?%A5%C6%A5%AF%A5%B9%A5%C1%A5%E3%A5%E1%A5%E2%A5%EA)

- CUDAではグローバルメモリ、ローカルメモリ、シェアードメモリの他に特殊なメモリとして、コンスタントメモリ、テクスチャメモリがある。これらのメモリはオンチップのキャッシュが用意されており、このキャッシュが効く限りは高速に実行できる。

### CUDA Array(`cudaArray`)を使用する手順
1. テクスチャリファレンスの作成（カーネルから読めるようにグローバル変数として）
2. CUDA Arrayの確保とホストからのデータ転送
3. テクスチャパラメータの設定
4. CUDA Arrayをテクスチャにバインド
5. カーネル内でtex2Dなどの参照関数を用いて値を取得

### 1. テクスチャリファレンスの作成
テクスチャの属性のいくつかはコンパイル時に既知である必要があるため、テンプレートを使ってテクスチャリファレンス変数宣言時に設定する。
```c++
