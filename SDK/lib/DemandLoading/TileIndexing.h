...
// clang-format off
#ifdef __CUDACC__
HOSTDEVICE static inline int ifloor( float x ) { return static_cast<int>( ::floorf( x ) ); }
HOSTDEVICE static inline float floorf( float x ) { return ::floorf( x ); }
HOSTDEVICE static inline float ceilf( float x ) { return ::ceilf( x ); }
HOSTDEVICE static inline float maxf( float x, float y ) { return ::fmaxf( x, y ); }
HOSTDEVICE static inline float minf( float x, float y ) { return ::fminf( x, y ); }
HOSTDEVICE static inline unsigned int uimax( unsigned int a, unsigned int b ) { return ( a > b ) ? a : b; }
#else
HOSTDEVICE static inline int ifloor( float x ) { return static_cast<int>( std::floor( x ) ); }
HOSTDEVICE static inline float floorf( float x ) { return std::floor( x ); }
HOSTDEVICE static inline float ceilf( float x ) { return std::ceil( x ); }
HOSTDEVICE static inline float maxf( float x, float y ) { return std::max( x, y ); }
HOSTDEVICE static inline float minf( float x, float y ) { return std::min( x, y ); }
HOSTDEVICE static inline unsigned int uimax( unsigned int a, unsigned int b ) { return std::max( a, b ); }
#endif
HOSTDEVICE static inline float clampf( float f, float a, float b ) { return maxf( a, minf( f, b ) ); }
// clang-format on

HOSTDEVICE static inline unsigned int ceilMult( unsigned int num, unsigned int den, float invDen )
{
    // This should work as long as (num + dem - 1) < 16M
    return static_cast<unsigned int>( static_cast<float>( num + den - 1 ) * invDen );
}

HOSTDEVICE static inline unsigned int calculateLevelDim( unsigned int mipLevel, unsigned int textureDim )
{
    return uimax( textureDim >> mipLevel, 1U );
}

HOSTDEVICE static unsigned int wrapPixelCoord( cudaTextureAddressMode addressMode, int coord, int max )
{
...
HOSTDEVICE static inline float wrapNormCoord( cudaTextureAddressMode addressMode, float x )
{
    if( addressMode == cudaAddressModeClamp || addressMode == cudaAddressModeBorder )
        return clampf( x, 0.0f, FIRST_FLOAT_LESS_THAN_ONE );  // result must be < 1
...
static const unsigned int MAX_TILES_CALCULATED = 4;
...
HOSTDEVICE static inline bool isMipTailIndex( unsigned int pageIndex )
{
