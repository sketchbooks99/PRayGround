...
{
    std::vector<shared_ptr<Shape>> shapes(1, shape);
...
void GeometryAccel::build(const Context& ctx, const std::vector<std::shared_ptr<Shape>>& shapes, uint32_t sbt_base)
{
    ...
    {
        Message(MSG_ERROR, "oprt::GeometryAccel::build(): The OptixBuildInputType of " \
         + "OPTIX_BUILD_INPUT_TYPE_INSTANCES or OPTIX_BUILD_INPUT_TYPE_INSTANCE_POINTERS " \
         + "cannot be used as an input type of geometry acceleration structure.");
        return;
    }
    ...
    std::vector<OptixBuildInput> built_inputs(shapes.size());
    m_options.operation = OPTIX_BUILD_OPERATION_BUILD;

    uint32_t sbt_idx = sbt_base;
    for (size_t i = 0; i < shapes.size(); i++)
    {
        shape[i]->buildInput(build_inputs[i], sbt_idx);
        sbt_idx++;
    }
    ...
    OPTIX_CHECK(optixAccelBuild(
        ...
        build_inputs.size(), 
        d_temp_buffer, 
        gas_buffer_temp_output_gas_and_compacted_size, 
    ...
    if (compacted_gas_size < gas_buffer_sizes.outputSizeInBytes) {
        ...
        OPTIX_CHECK(optixAccelCompact(static_cast<OptixDeviceContext>(ctx), 0, handle, d_buffer, compacted_gas_size, &m_handle));
...
{
    std::vector<OptixInstance> optix_instaces(instances.size());
    std::transform(instances.begin(), instances.end(), optix_instaces.begin(), 
...
void InstanceAccel::build(const Context& ctx, const std::vector<OptixInstance>& optix_instances)
{
    ...
    instance_input.type = m_type;
    instance_input.instanceArray.instances = d_instances.devicePtr();
    instance_input.numInstances = static_cast<uint32_t>(instances.size());
